...
class Instrument(db.Model):
	...
	def from_dict(self, data, partial_update=False):
		...
		for field in ['particle_id', 'latitude', 'longitude', 'location',
						...
						'timezone', 'nickname', 'private', 'type', 'model']:
			try:
				setattr(self, field, data[field])
			except KeyError:
				pass
	...
	def most_recent_datapoint(self):
		...
		q = m.query.filter(m.instr_sn==self.sn).order_by(m.timestamp.desc()).first_or_404()
...
class MITData(db.Model):
	...
	def to_json(self, researcher=False, **kwargs):
		...
		res['timestamp_local'] = to_timezone(self.timestamp, self.device.timezone)
		...
		res['pm10'] = self.pm10
		res['rh'] = self.rh_i
		res['temp'] = self.temp_i
		res['flag'] = self.flag

		if researcher is True:
			res['co_we'] = self.co_we
			res['co_ae'] = self.co_ae
			res['ox_we'] = self.ox_we
			res['ox_ae'] = self.ox_ae
			res['nox_we'] = self.nox_we
			res['nox_ae'] = self.nox_ae
			res['so2_we'] = self.so2_we
			res['so2_ae'] = self.so2_ae
			res['bin0'] = self.bin0
			res['bin1'] = self.bin1
			res['bin2'] = self.bin2
			res['bin3'] = self.bin3
			...
			res['bin8'] = self.bin8
			res['bin9'] = self.bin9
			res['bin10'] = self.bin10
			res['bin11'] = self.bin11
			res['bin12'] = self.bin12
			res['bin13'] = self.bin13
			res['bin14'] = self.bin14
			res['bin15'] = self.bin15
			res['bin1MToF'] = self.bin1MToF
			res['bin3MToF'] = self.bin3MToF
			res['bin5MToF'] = self.bin5MToF
			res['bin7MToF'] = self.bin7MToF
			res['period'] = self.period
			res['sfr'] = self.sfr
			res['cycles'] = self.cycles
			res['last_updated']  = self.last_updated
...
class EBamData(db.Model):
	...
	def to_json(self, researcher=False):
		return {
			...
			'unit': 'mg/m3',
			'parameter': 'pm25',
			'conc_rt': self.conc_rt,
			'conc_hr': self.conc_hr,
			'flowrate': self.flowrate,
			'wind speed': self.wind_speed,
			'wind direction': self.wind_dir,
			'ambient temperature': self.ambient_temp,
			'rh external': self.rh_external,
			'rh internal': self.rh_internal,
			'flag': self.flag
			}
...
class TrexData(db.Model):
	...
	def to_json(self, researcher=False):
		...
		d['timestamp_local'] = to_timezone(self.timestamp, self.device.timezone)
		d['so2'] = self.so2
		d['instrument'] = self.get_url()
		d['rh'] = self.rh
		d['temp'] = self.temp
		d['flag'] = self.flag

		if researcher:
			d['so2_we'] = self.so2_we
			d['so2_ae'] = self.so2_ae
